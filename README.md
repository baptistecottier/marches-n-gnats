# Marches & Gnats

All quests in the repository:

| # | Title                         | Status                                | Movements | Rank | Remark |
|---|-------------------------------|:-------------------------------------:|:----------|:----:|---------|
| 1 | Unary Addition                | [✅](./quests/quest_01/quest_01.rules)| 28 143    | #1
| 2 | Unary Even Odd                | [✅](./quests/quest_02/quest_02.rules)| 20 379    | #1
| 3 | Binary Increment              | [✅](./quests/quest_03/quest_03.rules)| 652       | #9
| 4 | Unary Multiplication          | [✅](./quests/quest_04/quest_04.rules)| 6 671     | #15  | length $\leq 20$
| 5 | Find Element in Unary Array   | [✅](./quests/quest_05/quest_05.rules)| 393 705   | #26  |
| 6 | Unary Subtraction             | [✅](./quests/quest_06/quest_06.rules)| 416 758   | #37
| 7 | Letter Mark                   | [✅](./quests/quest_07/quest_07.rules)| 83 776    | #23  | Rules pruned
| 8 | Text Mirror                   | [✅](./quests/quest_08/quest_08.rules)| 69 328    | #15
| 9 | Unary Comparison              | ❌                                    |
| 10| Lines Count                   | [✅](./quests/quest_10/quest_10.rules)| 848 715   | #29  |
| 11| Decimal Increment             | [✅](./quests/quest_11/quest_11.rules)| 444       | #6   |
| 12| Decimal Addition              | ❌                                    |
| 13| Unary Array Sort              | ❌                                    |
| 14| Binary to Decimal             | ❌                                    |
| 15| Square Root                   | ❌                                    |
| 16| Roman to Unary                | ❌                                    |
| 17| Cellular Automaton            | ❌                                    |
| 18| Unary Division                | ❌                                    |
| 20| Sudoku: Solver (Part II.)     | ❌                                    |
| 19| Sudoku: Validator (Part I.)   | ❌                                    |
| 21| Text Wrapping                 | ❌                                    |
| 22 | Vigenère Cipher              | ❌                                    |
| 23 | Two-Point Distance           | ❌                                    |
| 24 | Text Justification           | ❌                                    |
| 25 | French Number Spelling       | ❌                                    |
| 26 | Letter Frequency Sort        | ❌                                    |
| 27 | Unary Divisibility           | ❌                                    |
| 28 | Fibonacci Sequence           | ❌                                    |
| 29 | Advent of Logic Mill         | ❌                                    |
| 30 | Palindrome                   | ❌                                    |
| 31 | Unary to Decimal             | ❌                                    |
| 32 | Self-Counting                | ❌                                    |
| 33 | Valid Parentheses            | [✅](./quests/quest_33/quest_33.rules)| 6 942     | #9 
| 34 | Logic Gates                  | ❌                                    |
| 35 | Unary Deduplication          | ❌                                    |
| 36 | Abelian Sandpile             | ❌                                    |
| 37 | Greatest Common Divisor      | ❌                                    |
| 38 | Chess Check                  | ❌                                    |
| 39 | Merge Unary Arrays           | ❌                                    |
| 41 | Look and Say                 | ❌                                    |
| 40 | Escape Maze                  | ❌                                    |
| 42 | Toads and Frogs (PvP)        | ❌                                    |

## Usage

Run a quest by number with those generated test cases:

```bash
python3 mng.py [quest_number]
```
